...
function setScreen(name){screen=name;root.dataset.screen=name||'playing';for(const id of['menu','pause-screen','end-screen','help-screen'])$(id).hidden=id!==name;$('hud').hidden=name==='menu'||name==='help-screen';root.classList.toggle('playing',name!=='menu'&&name!=='help-screen');$('touch-controls').hidden=!touch||name!==null;if(name==='menu'||name==='end-screen'){levelBannerTimer=0;$('level-banner').classList.remove('show');}}
...
function startGame(){if(!ready)return;if(model.mode==='crashed'&&!endShown)finishRun();clearInput();audio.unlock().then(()=>audio.play('start'));model.start(saved.completedChallenges,saved.car);runUnlocks=[];particles=[];flash=0;endShown=false;crashTimer=0;levelBannerTimer=0;$('level-banner').classList.remove('show');world.shake=0;world.setCar(saved.car);setScreen(null);root.classList.remove('crashed');showCallout('LEVEL 1',`${model.car.name.toUpperCase()} · ${model.baseSpeed} KM/H`,'#fff',2.8);canvas.focus();updateHUD();}
function pauseGame(){if(!model.pause())return false;clearInput();setScreen('pause-screen');$('resume-button').focus();return true;}
function resumeGame(){if(!model.resume())return false;clearInput();audio.unlock();setScreen(null);canvas.focus();return true;}
function mainMenu(){model.reset(saved.car);world.setCar(saved.car);model.seedTraffic();model.playerX=2.7;clearInput();particles=[];flash=0;endShown=false;root.classList.remove('crashed','boosting','focused');setScreen('menu');updateRecords();$('start-button').focus();}
...
function finishRun(){
 ...
 const challenge=model.challenge;$('end-challenge').textContent=challenge.complete?`CHALLENGE COMPLETE · +${challenge.reward} POINTS`:`${challenge.label} · ${challenge.progress}/${challenge.target}`;$('end-challenge').classList.toggle('complete',challenge.complete);$('end-unlock').textContent=garageProgress();
 ...
 if(challenge.complete)tip=`NEXT RUN: ${createChallenge(saved.completedChallenges).label}.`;
 if(runUnlocks.length)tip=`${runUnlocks.at(-1).name} unlocked! Choose it in the garage.`;
 $('end-tip').textContent=tip;setScreen('end-screen');$('restart-button').focus();
}
...
function handleEvents(){for(const e of model.events){
 ...
 if(e.type==='challenge-complete'){
  ...
  if(recordChallenge(saved,model.challenge)){persist();updateRecords();}
  const unlocked=CARS.find(car=>!wasUnlocked.includes(car.id)&&carUnlocked(saved,car.id));if(unlocked)runUnlocks.push(unlocked);audio.play('level-up');showCallout('CHALLENGE COMPLETE',`+${e.points} · ${unlocked?unlocked.name.toUpperCase()+' UNLOCKED':'ONE MORE CHALLENGE COMPLETE'}`,'#ffd17b',2.4);
 }
...
function frame(now){requestAnimationFrame(frame);const dt=clamp((now-lastTime)/1000,0,.04);lastTime=now;if(!ready||contextLost)return;
 ...
 if(calloutTimer>0){calloutTimer-=dt;if(calloutTimer<=0)$('callout').classList.remove('show');}
...
function bindControls(){
 ...
 window.addEventListener('keydown',e=>{
  if(e.code==='Escape'||e.code==='KeyP'){e.preventDefault();if(screen==='help-screen')closeHelp();else if(model.mode==='paused')resumeGame();else pauseGame();return;}
  ...
  if(e.code==='KeyR'&&model.mode==='crashed'){e.preventDefault();startGame();}
